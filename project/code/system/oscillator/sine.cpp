...
void SYS_OSCILLATOR_SINE::Add(ID_SOURCE ID)
{
    SystemSources[SystemCount] = ID;
    ++SystemCount;
}

bool SYS_OSCILLATOR_SINE::Remove(ID_SOURCE ID)
{
...
void SYS_OSCILLATOR_SINE::Update(ENTITY_SOURCES *Sources, size_t BufferCount)
{
    ...
    for(size_t SystemIndex = 0; SystemIndex <= SystemCount; ++SystemIndex)
    {
        ...
        if(Source != 0)
        {
            ...
            if(Sources->Flags[SourceIndex] & ENTITY_SOURCES::MODULATOR)
            {
                CMP_MODULATOR &Modulator = Sources->Modulators[SourceIndex];
                RenderToBufferWithModulation(Sources->Oscillators[SourceIndex], Sources->Modulators[SourceIndex], Sources->Playbacks[SourceIndex].Buffer, BufferCount);
            }
            else
            {
