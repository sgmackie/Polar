...
void SYS_OSCILLATOR_SAWTOOTH::Create(MEMORY_ALLOCATOR *Allocator, size_t Size)
{
    SystemVoices = (ID_VOICE *) Allocator->Alloc((sizeof(ID_VOICE) * Size), HEAP_TAG_SYSTEM_OSC_SAWTOOTH);
    SystemCount = 0;
}

void SYS_OSCILLATOR_SAWTOOTH::Destroy(MEMORY_ALLOCATOR *Allocator)
{
    Allocator->Free(0, HEAP_TAG_SYSTEM_OSC_SAWTOOTH);
}

void SYS_OSCILLATOR_SAWTOOTH::Add(ID_VOICE ID)
{
    SystemVoices[SystemCount] = ID;
    ++SystemCount;
}

bool SYS_OSCILLATOR_SAWTOOTH::Remove(ID_VOICE ID)
{
...
void SYS_OSCILLATOR_SAWTOOTH::Update(ENTITY_VOICES *Voices, size_t BufferCount)
{
